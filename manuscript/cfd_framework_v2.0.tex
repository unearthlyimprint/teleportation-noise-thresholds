...
\end{equation}
The 4D integral and physical metric $g_{\mu\nu}$ are distinct from the 2D information-geometric parameter space $(p, \gamma)$; the action provides a template for how coherence-sourced stress-energy \emph{would} modify spacetime geometry if a microscopic embedding were established, where the coherence Lagrangian is:
\begin{equation}
...
\begin{table*}[t]
    ...
    \begin{tabular}{clcl}
        ...
        4 & Parametric geometry evolution & Preliminary$^*$ & Metric determinant tracking as $\gamma$ increases \\
        5 & Coherence-information decoupling & Open & Entanglement persistence at $\gamma \approx \gamma_c$ \\
        \bottomrule
    \end{tabular}
    \caption{Five predictions of CFD and proposed experimental tests. All are accessible on current NISQ hardware. $^*$Companion data~\cite{Arda2026companion} show consistency with threshold behavior, but results are also consistent with standard noise models.}
...
This prediction is supported by noiseless state-vector simulation of the 3-qubit teleportation circuit at $\gamma = 0.533 \approx \gamma_c$: the metric determinant $|\det(g)| = 0.0044$ (near-degenerate), yet the internal bipartite Bell state fidelity $F_{\text{Bell}} = 0.734$ remains super-classical ($F > 0.5$). Note that this measures the entanglement of the Bell bridge itself, not the teleportation fidelity (which approaches zero at the same $\gamma$); the distinction between these two fidelities is precisely the decoupling that Conjecture~3 predicts.
...
A major body of recent work establishes that horizons fundamentally \emph{cause} decoherence in nearby quantum systems. The DSW program~\cite{DSW2022, DSW2024} showed that any Killing horizon decoheres quantum superpositions at a constant rate via entanglement with interior degrees of freedom. This establishes a direction: horizons $\to$ decoherence.
...
\begin{enumerate}
    ...
    \item \textbf{Unitary vs.\ irreversible decoherence:} The companion experimental work~\cite{Arda2026companion} uses \emph{unitary} parametric dephasing ($R_z$ rotations), which is exactly reversible, whereas the CFD framework models \emph{irreversible} decoherence. The observed threshold behavior is consistent with CFD predictions, but a stronger test would use genuinely irreversible noise channels (e.g., amplitude damping) where the analogy to information loss is physically grounded.
    \item \textbf{Platform-dependent $\gamma_c$:} The companion work~\cite{Arda2026companion} finds $\gamma_c \approx 0.535$ on the IonQ gate-based platform and $\gamma_c \in [0.15, 0.30]$ on Pasqal's neutral-atom platform. This platform dependence is expected under CFD, since $\gamma_c$ depends on the system geometry (Eq.~\ref{eq:gamma_c}), but it also means that $\gamma_c$ is not a universal constant.
\end{enumerate}
...
The framework's distinguishing feature is testability: unlike approaches that require Planck-scale energies or astronomical observations, CFD predictions can be probed with quantum simulators. The most decisive near-term test is the entropy attenuation functional form: measuring $S_{\text{eff}}(\gamma)$ across a dense sweep and fitting to both the CFD exponential $S_0 e^{-\gamma/\gamma_c}$ and the standard dephasing polynomial $(1-\gamma)^n$ would provide---for the first time---a quantitative discriminator between CFD and standard dephasing models.
...
C.~Arda, ``Cross-platform noise thresholds in quantum teleportation: Trapped-ion and neutral-atom architectures,'' Zenodo (2026). doi:10.5281/zenodo.18743497.
...
\bibitem{DSW2022}
B.~S.~Danielson, G.~Satishchandran, and R.~M.~Wald, ``Killing horizons decohere quantum superpositions,'' Phys.\ Rev.\ Lett.\ \textbf{128}, 170401 (2022).
