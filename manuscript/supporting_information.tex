...
\begin{center}
\url{https://github.com/CelalArda/wormhole-cfd-stability}
\end{center}
...
\begin{table}[h]
    ...
    \begin{tabular}{ll}
        ...
        \midrule
        \multicolumn{2}{l}{\textit{Pasqal Neutral-Atom Emulator}} \\
        Atom species & $^{87}\text{Rb}$ \\
        Interaction type & Rydberg blockade (van der Waals) \\
        Emulator backends & \texttt{EMU\_FREE}, \texttt{EMU\_SV}, \texttt{EMU\_TN} \\
...
\begin{longtable}{p{0.42\textwidth}p{0.52\textwidth}}
    ...
    \midrule
    \endhead
    \texttt{code/experiment\_1\_phase\_transition.py} & IonQ simulator: sweeps $\gamma \in [0, 1]$ and records survival probability at each point. \\
    \texttt{code/experiment\_2\_active\_shielding.py} & Active Shielding protocol: tests fidelity recovery at $\gamma = 0.8$ with and without the inverse operator. \\
    \texttt{code/wormhole\_pulser\_continuous.py} & Continuous-mode Pulser simulation of the wormhole Hamiltonian. \\
    \texttt{scripts/tier1\_analysis.py} & Tier-1 result aggregation and statistical analysis. \\
    \texttt{scripts/tier1\_depth\_sweep.py} & Depth sweep experiment: varies circuit depth at fixed $\gamma$. \\
    \texttt{scripts/tier1v3\_trotter\_sweep.py} & Trotter-step scaling: varies Trotter steps from 1--8 on the 3-qubit architecture. \\
    \texttt{scripts/wormhole\_azure\_pasqal.py} & Bridge script for Azure $\leftrightarrow$ Pasqal execution. \\
    \texttt{scripts/wormhole\_control\_experiment.py} & Control experiment: teleportation circuit without Bell pair. \\
    \texttt{scripts/wormhole\_hardware\_correct.py} & Hardware-corrected protocol with classical post-processing. \\
    \texttt{scripts/wormhole\_teleport\_local\_test.py} & Local teleportation experiment with full shot-level analysis. \\
    \texttt{scripts/wormhole\_teleport\_sweep.py} & Parameter sweep of teleportation fidelity. \\
    \texttt{scripts/verify\_tensor\_derivation.py} & Numerical verification of tensor derivations in Section~2. \\
    \texttt{scripts/plot\_azure\_data.py} & Plotting utilities for Azure Quantum results. \\
    \texttt{pasqal\_native/scripts/run\_wormhole\_pasqal.py} & Pasqal Cloud submission: builds and submits sequences for $\gamma$ sweep. \\
    \texttt{pasqal\_native/scripts/run\_fine\_sweep.py} & Fine-grained $\gamma$ sweep near critical point. \\
    \texttt{pasqal\_native/scripts/run\_emulator\_comparison.py} & Cross-validation between \texttt{EMU\_FREE} and \texttt{EMU\_SV}. \\
    \texttt{pasqal\_native/scripts/analyze\_results.py} & Analysis and figure generation from emulator results. \\
    \texttt{pasqal\_native/scripts/merge\_results.py} & Merges multiple emulator result files. \\
    \texttt{pasqal\_native/scripts/run\_fresnel\_validation.py} & Fresnel validation of the neutral-atom layout. \\
    \bottomrule
\end{longtable}
