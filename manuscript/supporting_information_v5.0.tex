...
% Prefix SI equation numbers with 'S'
...
\begin{document}
	...
	\begin{center}
		\href{https://github.com/unearthlyimprint/teleportation-noise-thresholds}{GitHub: unearthlyimprint/teleportation-noise-thresholds}
	\end{center}
	...
	Table~\ref{tab:si_pasqal_sweep} presents the fine-grained dephasing sweep on the Pasqal neutral-atom platform using the \texttt{EMU\_FREE} emulator (42-atom layout, 8 $\gamma$ points $\times$ 500 shots = 4\,000 total shots). The transition region is clearly resolved between $\gamma = 0.15$ and $\gamma = 0.25$.
	...
	To verify simulation accuracy, we cross-validated the \texttt{EMU\_FREE} results against exact state-vector simulation (\texttt{EMU\_SV}) at three representative checkpoints. Table~\ref{tab:si_emulator_validation} confirms agreement within 1.5\% at all checkpoints, establishing that the computationally efficient \texttt{EMU\_FREE} backend faithfully reproduces the quantum dynamics.
	...
	\begin{table}[h]
		...
		\begin{tabular}{ccccc}
			...
			\bottomrule
		\end{tabular}
		\caption{FRESNEL\_CAN1 QPU hardware results (core 9-qubit extraction). The noise ratio quantifies $(1 - P_0^{\text{QPU}}) / (1 - P_0^{\text{ideal}})$. Note: Ideal EMU\_FREE baseline values differ from Table~\ref{tab:si_pasqal_sweep} due to the reduced spectator atom count in the 22-atom QPU layout (vs.\ 42 atoms in the emulator configuration).}
	...
	Table~\ref{tab:si_platforms} summarizes the specifications of both quantum platforms used in this study.
	...
	\begin{longtable}{p{0.45\textwidth}p{0.50\textwidth}}
		...
		\texttt{\small code/teleportation\_pulser\_continuous.py} & Continuous-mode Pulser simulation of the Rydberg Hamiltonian. \\
		...
		\texttt{\small scripts/teleportation\_control\_experiment.py} & Control: teleportation circuit without Bell pair. \\
		\texttt{\small scripts/teleportation\_hardware\_correct.py} & Hardware-corrected protocol with classical post-processing. \\
		\texttt{\small scripts/teleportation\_local\_test.py} & Local teleportation experiment with shot-level analysis. \\
		\texttt{\small scripts/teleportation\_sweep.py} & Parameter sweep of teleportation fidelity. \\
		...
		\texttt{\small pasqal\_native/scripts/\newline run\_teleportation\_pasqal.py} & Pasqal Cloud submission: builds and submits sequences for $\gamma$ sweep. \\
