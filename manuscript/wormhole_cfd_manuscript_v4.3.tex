...
\normalsize v4.3 --- Cross-Platform Emulator and Hardware Validation}
...
\begin{enumerate}
    ...
    \item \textbf{FRESNEL emulator validation:} The hardware-realistic \texttt{EMU\_FRESNEL} emulator, using a 42-atom register (FRESNEL layout) with 9~core qubits, confirms the collapse trend under physically faithful noise conditions ($P_0$: $26.6\% \to 89.4\%$ over $\gamma \in [0.05, 0.40]$). FRESNEL\_CAN1 QPU hardware jobs (22~atoms, 61-trap layout) are pending.
\end{enumerate}
...
\textbf{FRESNEL\_CAN1 QPU Hardware Validation.} Three jobs have been submitted to Pasqal's FRESNEL\_CAN1 neutral-atom QPU at $\gamma \in \{0.05, 0.20, 0.40\}$ (500~shots each, 22~atoms per sequence) and are currently pending in the execution queue. These will provide the first real neutral-atom hardware results for the CFD wormhole protocol, completing the validation chain: simulation (\texttt{EMU\_FREE}) $\to$ exact solver (\texttt{EMU\_SV}) $\to$ hardware emulator (\texttt{EMU\_FRESNEL}) $\to$ physical QPU (FRESNEL\_CAN1). Results will be incorporated upon completion.
